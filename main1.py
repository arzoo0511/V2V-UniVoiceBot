...
async def edge_text_to_speech(text):
    output_path = NamedTemporaryFile(suffix=".mp3", delete=False).name
    try:
...
def chatbot_pipeline(audio_path):
    try:
        ...
        response_audio = asyncio.run(edge_text_to_speech(response))
...
def update_ui(audio):
    try:
        ...
        audio_val = asyncio.run(edge_text_to_speech(reply_val))
        status_val = "Done"
...
def get_dummy_ip(): return "127.0.0.1"
